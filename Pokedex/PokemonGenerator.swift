            ...
            if let jsonData = NSData(contentsOfFile: path) {
                do {
                    ...
                    for key in json.keys {
                        ...
                        let flavorText = pokemonData["FlavorText"] as! String
                        pokemonArray.append(Pokemon(name: key, number: number, attack: attack, defense: defense, health: hp, spAttack: spatk, spDef: spdef, species: species, speed: speed, total: total, types: type, flavorText: flavorText))
                    }
                } catch {
